...
train.data <- RaModel("classification", data.ind, n = n, p = p)
test.data <- RaModel("classification", data.ind, n = n, p = p)
xtrain <- train.data$x
ytrain <- train.data$y
xtest <- test.data$x
ytest <- test.data$y
...
## auc : knn bad
## ric : not for knn,svm,tree

# apply when i <= 5
# fit_nocn <- RaSE(xtrain, ytrain, B1 = B1, B2 = B2,
#                  iteration = 0,base = as.character(ts_tab[i,1]))
# class(fit_nocn)
...
fit_0 <- RaSE(xtrain, ytrain, B1 = B1, B2 = B2,
              iteration = 0,base = as.character(ts_tab[i,1]),
              criterion = as.character(ts_tab[i,2]))
class(fit_0)
predict(fit_0,xtest)
...
fit_1 <- RaSE(xtrain, ytrain, B1 = B1, B2 = B2,
              iteration = 1,base = as.character(ts_tab[i,1]),
              criterion = as.character(ts_tab[i,2]))
class(fit_1)
predict(fit_1,xtest)
...
fit_2 <- RaSE(xtrain, ytrain, B1 = B1, B2 = B2,
              iteration = 2,base = as.character(ts_tab[i,1]),
              criterion = as.character(ts_tab[i,2]))
class(fit_2)
predict(fit_2,xtest)
...
fit_0 <- RaSE(xtrain, ytrain, B1 = B1, B2 = B2,
              iteration = 0,base = base,criterion = criterion[cn.no])
class(fit_0)
predict(fit_0,xtest)
mean(predict(fit_0,xtest) != ytest)
fit_0$ranking.base
