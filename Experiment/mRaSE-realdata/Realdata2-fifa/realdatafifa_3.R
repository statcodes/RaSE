...
spx <- spx + rnorm(length(spx), 0, 1e-3)
spx <- as.matrix(spx)
nmulti <- 4
ntrain <- nmulti*200
train_ind <- as.vector(sapply(1:nmulti, function(j){
  sample(which(spy == j), 200)
}))
nall <- nrow(spx)
...
file_name <- paste0(base_name,ntrain,"_",seed-1200-(set_id-1)*100,".RData")

setwd("/scratch/fb2234/Realdata2-result3")
