...
train.data1 <- RaModel("multi_classification", model.no = 4,
                       n = ntrain, p = p, p0 = p_seq)
test.data1 <- RaModel("multi_classification", model.no = 4,
                      n = ntest, p = p, p0 = p_seq)
xtrain1 <- train.data1$x
colnames(xtrain1) <- paste0("V",1:dim(xtrain1)[2])
ytrain1 <- train.data1$y
xtest1 <- test.data1$x
...
setwd("/scratch/fb2234/Simulation4-result")
